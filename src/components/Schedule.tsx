...
function getLevelColor(level: string) {
    switch (level) {
        ...
        case 'Intermediate':
            return 'bg-forge-orange/20 text-forge-orange'
        case 'Beginner':
            return 'bg-green-500/20 text-green-400'
        default:
            return 'bg-white/10 text-forge-silver'
    }
}
...
export default function Schedule() {
    ...
    return (
        <section
            id="schedule"
            className="relative py-16 md:py-20 bg-forge-black border-y border-white/5"
        >
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {scheduleData[activeDay]?.map((item, idx) => (
                        <div
                            key={idx}
                            className="bg-forge-gray p-4 border border-white/5 hover:border-forge-orange/30 transition-all cursor-pointer group"
                        >
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span
                                    className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 ${getLevelColor(item.level)}`}
                                >
                                    {item.level}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-3 pt-2 border-t border-white/5">
                                <span className="text-[10px] text-forge-silverDark">
                                    Coach: {item.coach}
                                </span>
                                <span
                                    className={`text-[10px] font-semibold ${item.spots <= 3 ? 'text-red-400' : 'text-forge-silver'}`}
                                >
