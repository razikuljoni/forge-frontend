      ...
      <div className="relative overflow-hidden bg-forge-gray">
        <div className="grid grid-cols-2">
          <div className="relative">
            <img src={story.beforeImg} alt="Before" className="w-full h-64 object-cover grayscale contrast-[1.2] transition-all duration-500 group-hover:grayscale-0" />
            <div className="absolute bottom-2 left-2 bg-forge-black/80 px-2 py-1">
              <span className="text-[9px] font-bold uppercase tracking-wider text-white">Before</span>
            </div>
          </div>
          <div className="relative">
            <img src={story.afterImg} alt="After" className="w-full h-64 object-cover" />
            <div className="absolute bottom-2 left-2 bg-forge-orange/90 px-2 py-1">
              <span className="text-[9px] font-bold uppercase tracking-wider text-forge-black">After</span>
            </div>
          </div>
        </div>
      </div>
      <div className="mt-4 space-y-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-forge-orange/20 border border-forge-orange/30 flex items-center justify-center rounded-full">
            <span className="font-heading font-bold text-forge-orange text-sm">{story.initials}</span>
          </div>
          <div>
            <div className="text-sm font-bold text-white">{story.name}</div>
            <div className="text-[10px] text-forge-silverDark">{story.role} • {story.age}</div>
          </div>
          <div className="ml-auto text-right">
            <div className="text-xs font-bold text-forge-orange">{story.weightLoss}</div>
            <div className="text-[10px] text-forge-silverDark">{story.duration}</div>
          </div>
        </div>
        <p className="text-xs text-forge-silver leading-relaxed italic">
          "{story.quote}"
        </p>
        <div className="flex gap-2">
          {story.tags.map(tag => (
            <span key={tag} className="text-[9px] font-semibold uppercase tracking-wider bg-forge-orange/10 text-forge-orange px-2 py-0.5">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-forge-orange">Real Transformations</span>
            </div>
            <h2 className="font-heading text-4xl md:text-5xl font-bold uppercase tracking-tight text-white">
              Their Words,<br />Our Proof
            </h2>
          </div>
          <p className="text-sm text-forge-silver max-w-md leading-relaxed">
            No Photoshop. No exaggeration. Real members, real dedication, real results. These are the stories that define FORGE.
          </p>
        </div>
